...
app.get("/api/export-excel/:type", async (req, res) => {
  try {
    ...
    if (type === "invoices") {
      data = await sageDbService.getInvoices();
      filename = `sage_invoices_${new Date().toISOString().split("T")[0]}.csv`;
      headers = [
        "AutoIndex",
        "OrderNum",
        "AccountID",
      ...
      data = await sageDbService.getInvoiceLines();
      filename = `sage_line_items_${
...
app.post("/test-webhook", async (req, res) => {
  ...
  try {
    ...
    const testPayload = {
      ...
      invoiceLines: [
        {
          cDescription: "UI Test Product",
          fQuantity: 1,
          fQtyToProcess: 1,
          fUnitPriceExclzDefault: 86.21,
          fUnitPriceInclzDefault: 100,
          fTaxRate: 16,
          iStockCodeID: 999,
          iTaxTypeID: 3,
          iWarehouseID: 1,
          bIsWhseItem: true,
          fQuantityLineTotExcl: 86.21,
          fQuantityLineTotIncl: 100,
          fQuantityLineTaxAmount: 13.79,
          iLineID: 1,
        },
      ],
      metadata: {
